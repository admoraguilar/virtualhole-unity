...
namespace FancyScrollView
{
	...
	public class Scroller : UIBehaviour, IPointerUpHandler, IPointerDownHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler
	{
		...
		/// </summary>
		public float DragScrollSensitivity
		{
			get => dragScrollSensitivity;
			set => dragScrollSensitivity = value;
		}
		...
		void IScrollHandler.OnScroll(PointerEventData eventData)
		{
			...
			var position = currentPosition + scrollDelta / ViewportSize * mouseScrollSensitivity;
		...
		void IDragHandler.OnDrag(PointerEventData eventData)
		{
			...
			if(movementType == MovementType.Elastic) {
				if(offset != 0f) {
					position -= RubberDelta(offset, dragScrollSensitivity);
				}
			}
		...
		void IEndDragHandler.OnEndDrag(PointerEventData eventData)
		{
			...
			var flickRegisterTime = .13f;

			if(dragTime <= flickRegisterTime) {
				flickDirection = scrollDirection == ScrollDirection.Horizontal ? -eventData.delta.x : eventData.delta.y;

				if(flickDirection != 0f) {
					bool isSameDirection = Mathf.Sign(velocity) == Mathf.Sign(flickDirection);
					if(isSameDirection) { velocity += CalculateFlickVelocity(); } 
					else { velocity = CalculateFlickVelocity(); }
				}

				float CalculateFlickVelocity()
				{
					var flickDirectionMinCoefficient = .1f;
					var flickDirectionMaxCoefficient = .20f;
					var flickDirectionScreenDeltaRatioMin = .5f;
					var flickDirectionScreenDeltaRatioMax = 15f;

					var flickDirectionAbs = Mathf.Abs(flickDirection);
					var flickScreenDeltaRatio = scrollDirection == ScrollDirection.Horizontal ?
					...
					return flickDirection * coefficient;
				}
