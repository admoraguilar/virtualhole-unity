...
{
 /**
  * LayoutAlgorithm v2.3.3
  *
  * Main changes:
  *  - April 2020: Fixed a bug in GROW ui's that were sometimes SHRINKING width/height constrained elements
  */
 [CreateAssetMenu(menuName = "Flexbox/Layout Algorithms/v2.3.3", fileName = "Layout Algorithm v2.3.3.asset", order=101)]
 public class FlexboxLayoutAlgorithm233 : IFlexboxLayoutAlgorithm
 {
  public override string defaultAssetName
  {
   get { return "Layout Algorithm v2.3.3"; }
  }
 ...
 private int _layoutsStartedUnfinished = 0;
  ...
  private Dictionary<FlexItem,Vector2> CalculateAndSetAllChildSizes( FlexLayoutData d, FlexContainer container, out float unusedMainSpace )
  {
   ...
   if( leftoverSpace > 0 )
   {
    grownAndShrunkDeltaLengths = ResolveGrowSizes(d, leftoverSpace, childBaseMainLengths, out unusedMainSpace, container ); // NOTE: leftoverspace was pre-calc'd using HYPOTHETICAL lengths, but grow/shrink only uses the BASE ones
   }
   ...
   foreach( var child in d.childItems )
   {
    ...
    if( container.showDebugMessages )
     Debug.Log("["+container.name+"] child: "+child.name+".mainLengthFinal = "+(childHypotheticalMainLengths[child])+" + "+(grownAndShrunkDeltaLengths[child])+" = "+finalLengths[child]);
   }
   ...
   foreach( var child in d.childItems )
     {
       ...
       */
      if( container.showDebugMessages )
       Debug.Log("["+container.name+"] child: "+child.name+".margins = "+child.cssMargins.ValueOrZero(d.axisMain, d.innerContentSize)+" (will subtract from main)");
  ...
  private static Dictionary<FlexItem, float> ResolveGrowSizes( FlexLayoutData d, float excessToFill, Dictionary<FlexItem, float> baseLengths, out float unusedSpace, FlexContainer containerForDebug = null )
  {
   ...
   if( containerForDebug != null && containerForDebug.showDebugMessages )
    Debug.Log("[" + containerForDebug.name + "] initial space to grow into = "+excessToFill );
   ...
   if( totalGrowWeight <= 0f ) // all items refused to grow
   {
    ...
    if( containerForDebug != null && containerForDebug.showDebugMessages )
     Debug.Log("[" + containerForDebug.name + "] ...all childitems are locked, can't grow any, quitting.");
    return growDeltas;
   }
   ...
   while( growingChildren.Count > 0 && allocatingExcess > 0 )
   {
    for( int i = 0; i < growingChildren.Count; i++ )
    {
     ...
     CSS3Length cssMax = child.cssMax(d.axisMain);
       ...
       if( containerForDebug != null && containerForDebug.showDebugMessages )
        Debug.Log("[" + containerForDebug.name + "] allocating "+acceptedExcess+" GROW to child = "+child.name );
     ...
     if( acceptedExcess < offeredExcess ) //if we hit a "max violation" as spec calls it, we return some of the excess, and "freeze" (spec's wording) the flexitem
     {
      ...
      if( containerForDebug != null && containerForDebug.showDebugMessages )
       Debug.Log("[" + containerForDebug.name + "] re-allocating "+unusedExcess+" to GROW pool, child = "+child.name );
