...
namespace Flexbox4Unity
{
  ...
  */
 [CreateAssetMenu(menuName = "Flexbox/Layout Algorithms/v3.4.0 beta", fileName = "Layout Algorithm v3.4.0-beta.asset")]
 public class FlexboxLayoutAlgorithm340 : IFlexboxLayoutAlgorithmV3
 {
  public override string defaultAssetName
  {
   get { return "Layout Algorithm v3.4.0"; }
  }
  private static readonly ReadOnlyCollection<string> _featureList = new ReadOnlyCollection<string>(
   new List<string> 
   {
    "TODO: FlexContainer magic-class support for RootFlexContainer special class (improves integration with UnityUI Scrollviews)",
   }
  );
  ...
  public override void AlgorithmLayout(FlexContainer container, Vector2 availableSize)
  {
   ...
   try
   {
    ...
    finally
    {
     ...
     /**
      * ---- If this was a RootFlexContainer, then special case: we'll also resize the container itself! to match it's children's sizes ----
      */
     if( rfc != null )
     {
      Vector2 totalContentSize = Vector2.zero;
      foreach( var child in d.childItems )
      {
       if( d.axisMain == Axis.Horizontal )
       {
        totalContentSize.x += (child.transform as RectTransform).rect.width;
        totalContentSize.y = Mathf.Max( totalContentSize.y, (child.transform as RectTransform).rect.height );
       }
       else
       {
        totalContentSize.x = Mathf.Max( totalContentSize.x, (child.transform as RectTransform).rect.width );
  ...
  private FlexLayoutData FlexLayoutSetup(FlexContainer container, Vector2 availableSize)
  {
   ...
   RootFlexContainer rootContainer = container as RootFlexContainer;
 ...
 */
   ...
   */
    ...
    */
    ...
    if( container is RootFlexContainer )
     leftoverSpace = 0;
