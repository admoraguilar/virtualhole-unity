...
{
 /**
  * LayoutAlgorithm v2.3
  *
  * Main changes:
  *  - completely re-wrote/fixed complicated margins and padding cases
  *  - fully implemented the logic behind flex = AUTO (automatic behaviour covers most/all common cases intelligently) 
  *  - fixed the use of CONTENT BOX vs BORDER BOX to correctly read the sizes as per spec throughout
  *  - made sure the layout algorithm is using correct definitions of CONTENT/BORDER and INNER/OUTER-CONTENT throughout
  *  - changing the MARGINS/PADDING on a BORDERBOX will automatically change its SIZE and IT will re-layout but its PARENT will not
  *  - changing the MARGINS/PADDING on a CONTENTBOX will acutomatially change its flex-SIZE and its PARENT will relayout, and it will too
  */
 [CreateAssetMenu(menuName = "Flexbox/Layout Algorithms/v2.3", fileName = "Layout Algorithm v2.3.asset", order = 105)]
 public class FlexboxLayoutAlgorithm23 : IFlexboxLayoutAlgorithm
 {
  public override string defaultAssetName
  {
   get { return "Layout Algorithm v2.3"; }
  }
 ...
 private int _layoutsStartedUnfinished = 0;
  ...
  private void LayoutWithinCurrentSize( FlexContainer container )
  {
    ...
    */
   List<FlexItem> unityArrayAsList = new List<FlexItem>(d.childItems);
   List<FlexItem> flexOrderedChildren = new List<FlexItem>(d.childItems);
