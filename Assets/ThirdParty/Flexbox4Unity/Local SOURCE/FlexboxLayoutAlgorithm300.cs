...
{
 /**
  * LayoutAlgorithm v3.0.0
  *
  * Main changes:
  *  - adding feature: flex-wrap
  *  - FIXED: many subtle bugs with sizing rows-of-cols, or cols-of-rows
  *  - FIXED: many subtle bugs with containers that didn't use STRETCH (the default)
  *  - FIXES: bug where flex-grow items could actually SHRINK in some cases (!)
  *  - FIXED: bug where flex-shrink items could sometimes reset and then grow while their neighbours were shrinking
  *  - FIXES: disabled UnityUI Components were still contributing to the layout of AUTO flexitems
  *  - FIXED: margins incorrect in some complex situations with nested containers and AUTO-sized children with over-large content
  */
 [CreateAssetMenu(menuName = "Flexbox/Layout Algorithms/v3.0.0", fileName = "Layout Algorithm v3.0.0.asset")]
 public class FlexboxLayoutAlgorithm300 : IFlexboxLayoutAlgorithmV3
 {
  public override string defaultAssetName
  {
   get { return "Layout Algorithm v3.0.0"; }
  }
  ...
  public override void AlgorithmLayout(FlexContainer container, Vector2 availableSize)
  {
   ...
   Dictionary<FlexItem, Vector2> finalSizes = CalculateAndSetAllChildSizes(d, container, out usedMainSpacePerLine, out unusedMainSpacePerLine, out flexLines, out flexLineCrossHeights );
   ...
   foreach( var child in flexLine )
   {
    ...
    if( container.showDebugMessages ) Debug.Log("[" + container.name + "." + child.name + "] position will be: **[ " + mainAxisPosition + "] ** <== "
  ...
  private Vector2 ContentSize_AContainer(FlexContainer element, Vector2 containingBlocksizeForOrthogonalFLow )//, VectorNullable2 elementFixedSize ) // TODO: rename 2nd arg to "availableSpaceInParent_orSelfFixedSize" or similar
  {
   ...
   if( element.showDebugMessages ) Debug.Log("["+element.name+"] Calculating self-size as a CONTENT-mode container; with: "+children.Count
   ...
   if( element.showDebugMessages )
    for( int lineIndex=0; lineIndex<flexLines.Count; lineIndex++ ) Debug.Log("[" + element.name + "] .. as a CONTENT-mode container, found: " + flexLines.Count + " flexlines, line["+lineIndex+"] has cross-length = " + lineBaseCrossLengths[flexLines[lineIndex]]);
